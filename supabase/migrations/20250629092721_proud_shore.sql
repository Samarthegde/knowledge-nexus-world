/*
  ...
  1. New Tables
    - `ai_conversations` - Store AI chat conversations for context and analytics
    - `ai_usage_analytics` - Track AI usage patterns and effectiveness
    - `learning_preferences` - Store student learning preferences for AI personalization
...
INSERT INTO learning_preferences (user_id, learning_style, explanation_preference, interaction_style)
SELECT 
  id,
  'adaptive',
  'balanced',
  'encouraging'
FROM auth.users
WHERE id NOT IN (SELECT user_id FROM learning_preferences)
